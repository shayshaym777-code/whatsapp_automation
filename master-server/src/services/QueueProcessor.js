...
class QueueProcessor {
    ...
    async processQueue() {
        ...
        try {
            ...
            const MESSAGES_PER_MINUTE_PER_SENDER = 15; // 15 messages per minute per sender
            ...
            logger.info(`[QueueProcessor] 📊 Capacity: ${totalCapacity} msg/min (${availableSenders.length} × ${MESSAGES_PER_MINUTE_PER_SENDER}) | Used: ${totalUsed}/${totalCapacity} | Remaining: ${totalRemaining}`);
    ...
    async getAvailableSenders() {
        ...
        for (const account of allAccounts) {
            try {
                ...
                // 3. Not over 15 messages per minute (limit per sender)
