...
CREATE TABLE IF NOT EXISTS message_queue (
    ...
    -- Higher = more priority (existing chat = 10, new = 0)
    status VARCHAR(20) DEFAULT 'pending',
...
SELECT a.phone,
    ...
    CASE
        ...
        AND a.blocked_at > NOW() - INTERVAL '48 hours' THEN 'BLOCKED'
...
CREATE OR REPLACE FUNCTION get_healthy_accounts() RETURNS TABLE(
        phone VARCHAR(20),
        country VARCHAR(2),
        proxy_id VARCHAR(50),
...
WHERE a.blocked_at IS NULL
    OR a.blocked_at < NOW() - INTERVAL '48 hours'
GROUP BY a.phone,
    a.country,
    a.proxy_id
