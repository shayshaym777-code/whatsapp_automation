...
CREATE TABLE IF NOT EXISTS workers (
    ...
    CONSTRAINT workers_status_check CHECK (status IN ('online', 'offline', 'busy', 'error', 'maintenance'))
);
...
CREATE TABLE IF NOT EXISTS accounts (
    ...
    CONSTRAINT accounts_status_check CHECK (status IN ('connected', 'disconnected', 'connecting', 'qr_pending', 'banned', 'error')),
    CONSTRAINT accounts_trust_score_check CHECK (trust_score >= 0 AND trust_score <= 100)
);
...
CREATE TABLE IF NOT EXISTS messages (
    ...
    CONSTRAINT messages_status_check CHECK (status IN ('queued', 'processing', 'sent', 'delivered', 'read', 'failed', 'cancelled')),
    CONSTRAINT messages_priority_check CHECK (priority IN ('high', 'normal', 'low')),
    CONSTRAINT messages_type_check CHECK (message_type IN ('text', 'image', 'video', 'audio', 'document', 'location', 'contact'))
);
...
CREATE INDEX IF NOT EXISTS idx_messages_queued ON messages(status, priority, created_at) WHERE status = 'queued';
...
CREATE TABLE IF NOT EXISTS campaigns (
    ...
    CONSTRAINT campaigns_status_check CHECK (status IN ('draft', 'scheduled', 'running', 'paused', 'completed', 'cancelled', 'failed'))
);
...
CREATE TABLE IF NOT EXISTS proxies (
    ...
    CONSTRAINT proxies_status_check CHECK (status IN ('available', 'assigned', 'testing', 'failed', 'disabled')),
    CONSTRAINT proxies_type_check CHECK (proxy_type IN ('http', 'https', 'socks4', 'socks5')),
...
CREATE INDEX IF NOT EXISTS idx_proxies_available ON proxies(country, status) WHERE status = 'available' AND is_active = TRUE;
