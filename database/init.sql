...
CREATE TABLE IF NOT EXISTS accounts (
    ...
    is_new BOOLEAN DEFAULT FALSE,           -- v7.0: Manual flag for new device warmup
    stage VARCHAR(20) DEFAULT 'Adult',      -- v7.0: Baby, Toddler, Teen, Adult, Veteran
    power INTEGER DEFAULT 100,              -- v7.0: Power score for load distribution
    max_per_day INTEGER DEFAULT 100,        -- Daily message limit based on stage
...
CREATE INDEX IF NOT EXISTS idx_accounts_is_new ON accounts(is_new) WHERE is_new = TRUE;

-- ============================================
-- SESSIONS TABLE (v7.0 NEW)
...
CREATE TABLE IF NOT EXISTS send_log (
    id SERIAL PRIMARY KEY,
    phone VARCHAR(20) NOT NULL,             -- Sender phone
    recipient VARCHAR(20) NOT NULL,         -- Recipient phone
    content TEXT,
...
CREATE TABLE IF NOT EXISTS proxies (
    ...
    country VARCHAR(2) NOT NULL,            -- US, IL, GB
    ...
    assigned_phone VARCHAR(20),             -- Which phone is using this proxy
...
CREATE INDEX IF NOT EXISTS idx_proxies_active ON proxies(is_active) WHERE is_active = TRUE;
CREATE INDEX IF NOT EXISTS idx_proxies_available ON proxies(country, is_active) 
    WHERE is_active = TRUE AND assigned_phone IS NULL;
...
CREATE TABLE IF NOT EXISTS warmup_stages (
    ...
    power INTEGER NOT NULL,                 -- v7.0: Power score
    description TEXT
);

-- Insert v7.0 warmup stages
INSERT INTO warmup_stages (name, min_days, max_days, daily_limit, power, description)
VALUES 
    ('WARMING', 1, 3, 5, 0, 'Day 1-3: Only internal warmup, no campaigns'),
    ('Baby', 4, 7, 15, 15, 'Day 4-7: Light activity'),
    ('Toddler', 8, 14, 30, 30, 'Day 8-14: Moderate activity'),
    ('Teen', 15, 30, 50, 50, 'Day 15-30: Normal activity'),
    ('Adult', 31, 60, 100, 100, 'Day 31-60: Full activity'),
    ('Veteran', 61, 9999, 200, 200, 'Day 60+: Maximum capacity')
...
CREATE OR REPLACE FUNCTION count_connected_sessions(p_phone VARCHAR(20)) 
RETURNS INTEGER AS $$
DECLARE
...
BEGIN
    ...
    FROM sessions
    WHERE phone = p_phone AND status = 'CONNECTED';
...
INSERT INTO workers (worker_id, host, port, proxy_country, max_accounts, status, device_seed)
VALUES 
    ('worker-1', 'worker-1', 3001, 'US', 10, 'offline', 'unique-seed-worker-1-usa-abc123'),
    ('worker-2', 'worker-2', 3001, 'IL', 10, 'offline', 'unique-seed-worker-2-israel-xyz789'),
    ('worker-3', 'worker-3', 3001, 'GB', 10, 'offline', 'unique-seed-worker-3-uk-qwe456'),
    ('worker-4', 'worker-4', 3001, 'US', 10, 'offline', 'unique-seed-worker-4-usa-def456')
