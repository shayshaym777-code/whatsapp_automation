...
export const WORKERS = [
  { 
    ...
    flag: '🇺🇸',
    port: 3001,
    url: 'http://localhost:3001'  // Direct access for dev, use nginx proxy in prod
  },
  { 
    id: 'worker-2', 
    name: 'Worker 2', 
...
function getWorkerUrl(worker) {
  // In production (Docker), use internal network
  if (typeof window !== 'undefined' && window.location.hostname !== 'localhost') {
    return `http://${window.location.hostname}:${worker.port}`
  }
  return worker.url
}
